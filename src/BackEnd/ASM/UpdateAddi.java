	...
	public void process(){
		...
		for(ASMFunction currentfunction:asmmodule.funcList){
			for(ASMBasicBlock currentblock:currentfunction.blockList){
				...
				while(iterator.hasNext()) {
					Base_Inst_ASM inst = iterator.next();
					if(preinst!=null && preinst instanceof Li_Inst_ASM && preinst.imm.imm<2048 && inst instanceof Binary_Inst_ASM && (((Binary_Inst_ASM) inst).op.equals("add") || ((Binary_Inst_ASM) inst).op.equals("sub"))){
